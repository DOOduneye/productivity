...
export class MilestoneController {
  ...
  createMilestoneTask = async (req: Request, res: Response) => {
    try {
      ...
      const task = await this.milestoneService.createMilestoneTask(Number(milestoneId), req.body);
  ...
  updateMilestone = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const milestone = await this.milestoneService.updateMilestone(Number(id), req.body);
      res.status(200).json(milestone);
    } catch (error) {
